...
- **SM syslog stream is an independent SIP transaction record useful for ruling out pcap coverage gaps**: Avaya SM forwards every SIP message it processes to a syslog target (typically UDP from SM IP to log server on `:514` or non-standard high port). Each syslog packet contains the full SIP message in payload (sometimes spanning 2–3 UDP packets — group by `(src_port, dst_port)` tuple to reassemble). Absence of a given SIP transaction in SM syslog is strong evidence it was never generated by the upstream component, ruling out "pcap missed that hop." Reference: SR `1-23647477802` — SM `PVCLIPBASM0031H` syslog at `172.17.61.10 → 172.17.65.96/.97` corroborated absent CSeq:2 INVITE on BC1/BC2 dialogs.
